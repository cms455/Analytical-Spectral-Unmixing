...
% Envelope smoothing
% === Step 4: Polynomial Fit instead of Envelope ===
x = (1:length(selection_counts))';
y = selection_counts(:);

poly_order = 9;  % You can adjust to 6, 8, 9 based on smoothness/fit quality
[p_fit, gof] = fit(x, y, sprintf('poly%d', poly_order));
y_fit = feval(p_fit, x);

% Residual = difference from the smooth fit
residuals = abs(y - y_fit);  % This acts as the 'envelope diff'

% Plot residuals
figure;
plot(y_fit, 'k', 'LineWidth', 1.5);
title(sprintf('Residual from Polynomial Fit (Degree %d)', poly_order));
xlabel('Column Index'); ylabel('Deviation from Fit');
