function [best_combo, best_norm] = og_dist_v2(A, k, num_trials, neigh_range, num_neigh_iters)
num_cols = size(A, 2); 
% Random sampling
...
y_grad_smooth = smoothdata(abs(gradient(y_smooth)),"gaussian",20);
...
figure;
plot(y_grad_smooth, 'k', 'LineWidth', 1.5);
title('Gradient Smoothed Selection Frequency');
xlabel('Column Index'); ylabel('Frequency');
%}

env_diff = y_grad_smooth;
