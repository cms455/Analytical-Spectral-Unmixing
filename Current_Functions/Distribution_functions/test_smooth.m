...
% Smooth the selection count distribution using a lowess fit or moving average
x = (1:num_cols)';
y = selection_counts(:);

% Choose smoothing method
smooth_span = 0.15;  % e.g., 15% of the data
y_smooth = smoothdata(y);

% Residuals from the smooth trend
residuals = abs(y - y_smooth);
...
% Report result
no_neigh_norm = norm(pinv(A(:, no_neigh_combo)));
